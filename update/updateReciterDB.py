...
def load_person_article1(mysql_cursor):
    ...
    load_person_article1_query = (
        ...
        "(personIdentifier, pmid, @authorshipLikelihoodScore, pmcid, @totalArticleScoreStandardized, @totalArticleScoreNonStandardized, "
        "userAssertion, publicationDateDisplay, publicationDateStandardized, publicationTypeCanonical, scopusDocID, journalTitleVerbose, "
        "articleTitle, @feedbackScoreAccepted, @feedbackScoreRejected, @feedbackScoreNull, articleAuthorNameFirstName, articleAuthorNameLastName, "
        "institutionalAuthorNameFirstName, institutionalAuthorNameMiddleName, institutionalAuthorNameLastName, @nameMatchFirstScore, nameMatchFirstType, "
        "@nameMatchMiddleScore, nameMatchMiddleType, @nameMatchLastScore, nameMatchLastType, @nameMatchModifierScore, @nameScoreTotal, emailMatch, "
        "@emailMatchScore, journalSubfieldScienceMetrixLabel, @journalSubfieldScienceMetrixID, journalSubfieldDepartment, @journalSubfieldScore, "
        "@relationshipEvidenceTotalScore, @relationshipMinimumTotalScore, @relationshipNonMatchCount, @relationshipNonMatchScore, articleYear, "
        "identityBachelorYear, discrepancyDegreeYearBachelor, @discrepancyDegreeYearBachelorScore, identityDoctoralYear, discrepancyDegreeYearDoctoral, "
        "@discrepancyDegreeYearDoctoralScore, @genderScoreArticle, @genderScoreIdentity, @genderScoreIdentityArticleDiscrepancy, personType, @personTypeScore, "
        "@countArticlesRetrieved, @articleCountScore, targetAuthorInstitutionalAffiliationArticlePubmedLabel, @pubmedTargetAuthorInstitutionalAffiliationMatchTypeScore, "
        "scopusNonTargetAuthorInstitutionalAffiliationSource, @scopusNonTargetAuthorInstitutionalAffiliationScore, @totalArticleScoreWithoutClustering, "
        "@clusterScoreAverage, @clusterReliabilityScore, @clusterScoreModificationOfTotalScore, datePublicationAddedToEntrez, clusterIdentifier, doi, issn, issue, "
        "journalTitleISOabbreviation, pages, timesCited, volume, @feedbackScoreCites, @feedbackScoreCoAuthorName, @feedbackScoreEmail, @feedbackScoreInstitution, "
        "@feedbackScoreJournal, @feedbackScoreJournalSubField, @feedbackScoreKeyword, @feedbackScoreOrcid, @feedbackScoreOrcidCoAuthor, @feedbackScoreOrganization, "
        "@feedbackScoreTargetAuthorName, @feedbackScoreYear) "
        "SET "
        "authorshipLikelihoodScore = NULLIF(@authorshipLikelihoodScore, ''), "
        "totalArticleScoreStandardized = NULLIF(@totalArticleScoreStandardized, ''), "
        "totalArticleScoreNonStandardized = NULLIF(@totalArticleScoreNonStandardized, ''), "
        "feedbackScoreAccepted = NULLIF(@feedbackScoreAccepted, ''), "
        "feedbackScoreRejected = NULLIF(@feedbackScoreRejected, ''), "
        "feedbackScoreNull = NULLIF(@feedbackScoreNull, ''), "
        "nameMatchFirstScore = NULLIF(@nameMatchFirstScore, ''), "
        "nameMatchMiddleScore = NULLIF(@nameMatchMiddleScore, ''), "
        "nameMatchLastScore = NULLIF(@nameMatchLastScore, ''), "
        "nameMatchModifierScore = NULLIF(@nameMatchModifierScore, ''), "
        "nameScoreTotal = NULLIF(@nameScoreTotal, ''), "
        "emailMatchScore = NULLIF(@emailMatchScore, ''), "
        "journalSubfieldScienceMetrixID = NULLIF(@journalSubfieldScienceMetrixID, ''), "
        "journalSubfieldScore = NULLIF(@journalSubfieldScore, ''), "
        "relationshipEvidenceTotalScore = NULLIF(@relationshipEvidenceTotalScore, ''), "
        "relationshipMinimumTotalScore = NULLIF(@relationshipMinimumTotalScore, ''), "
        "relationshipNonMatchCount = NULLIF(@relationshipNonMatchCount, ''), "
        "relationshipNonMatchScore = NULLIF(@relationshipNonMatchScore, ''), "
        "discrepancyDegreeYearBachelorScore = NULLIF(@discrepancyDegreeYearBachelorScore, ''), "
        "discrepancyDegreeYearDoctoralScore = NULLIF(@discrepancyDegreeYearDoctoralScore, ''), "
        "genderScoreArticle = NULLIF(@genderScoreArticle, ''), "
        "genderScoreIdentity = NULLIF(@genderScoreIdentity, ''), "
        "genderScoreIdentityArticleDiscrepancy = NULLIF(@genderScoreIdentityArticleDiscrepancy, ''), "
        "personTypeScore = NULLIF(@personTypeScore, ''), "
        "countArticlesRetrieved = NULLIF(@countArticlesRetrieved, ''), "
        "articleCountScore = NULLIF(@articleCountScore, ''), "
        "pubmedTargetAuthorInstitutionalAffiliationMatchTypeScore = NULLIF(@pubmedTargetAuthorInstitutionalAffiliationMatchTypeScore, ''), "
        "scopusNonTargetAuthorInstitutionalAffiliationScore = NULLIF(@scopusNonTargetAuthorInstitutionalAffiliationScore, ''), "
        "totalArticleScoreWithoutClustering = NULLIF(@totalArticleScoreWithoutClustering, ''), "
        "clusterScoreAverage = NULLIF(@clusterScoreAverage, ''), "
        "clusterReliabilityScore = NULLIF(@clusterReliabilityScore, ''), "
        "clusterScoreModificationOfTotalScore = NULLIF(@clusterScoreModificationOfTotalScore, ''), "
        "feedbackScoreCites = NULLIF(@feedbackScoreCites, ''), "
        "feedbackScoreCoAuthorName = NULLIF(@feedbackScoreCoAuthorName, ''), "
        "feedbackScoreEmail = NULLIF(@feedbackScoreEmail, ''), "
        "feedbackScoreInstitution = NULLIF(@feedbackScoreInstitution, ''), "
        "feedbackScoreJournal = NULLIF(@feedbackScoreJournal, ''), "
        "feedbackScoreJournalSubField = NULLIF(@feedbackScoreJournalSubField, ''), "
        "feedbackScoreKeyword = NULLIF(@feedbackScoreKeyword, ''), "
        "feedbackScoreOrcid = NULLIF(@feedbackScoreOrcid, ''), "
        "feedbackScoreOrcidCoAuthor = NULLIF(@feedbackScoreOrcidCoAuthor, ''), "
        "feedbackScoreOrganization = NULLIF(@feedbackScoreOrganization, ''), "
        "feedbackScoreTargetAuthorName = NULLIF(@feedbackScoreTargetAuthorName, ''), "
        "feedbackScoreYear = NULLIF(@feedbackScoreYear, '')"
    )
...
def load_person_article2(mysql_cursor):
    ...
    load_person_article2_query = (
        "LOAD DATA LOCAL INFILE '" + cwd + "/temp/parsedOutput/person_article2.csv' INTO TABLE person_article "
        "FIELDS TERMINATED BY ',' ENCLOSED BY '\"' IGNORE 1 LINES "
        "(personIdentifier, pmid, @authorshipLikelihoodScore, pmcid, @totalArticleScoreStandardized, @totalArticleScoreNonStandardized, "
        "userAssertion, publicationDateDisplay, publicationDateStandardized, publicationTypeCanonical, scopusDocID, journalTitleVerbose, "
        "articleTitle, @feedbackScoreAccepted, @feedbackScoreRejected, @feedbackScoreNull, articleAuthorNameFirstName, articleAuthorNameLastName, "
